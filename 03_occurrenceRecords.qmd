...
This will bring up a file called **.Renviron**. Add your own credential information here (replace the info in quotes with your info), or you can use the account info that we've provided for this workshop.
...
## Create occurence grid for _Bombus sitkensis_
...
occ.envr <- rast(list(prmean, tmean_yr, elevcc, forageFall,
                      forageSpring, forageSummer, nesting, insecticide, occ.rast))


presence <- values(occ.envr, dataframe = TRUE, na.rm = TRUE) |> filter(B_sitkensis == 1)

```

## Create background points grid

Background samples are obtained randomly and independently of species locations. Background points are meant to sample the full environmental space in the region. This means that background points:

- should be numerous enough to represent the region
...
background <- extract(occ.envr[[1:9]],    # only extracting the environmental/habitat layers
