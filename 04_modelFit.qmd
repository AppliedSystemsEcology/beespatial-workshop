...
In this module, you'll build and evaluate a species distribution model
for *Bombus sitkensis* using a Random Forest algorithm. You'll also
generate spatial predictions, evaluate model performance, and visualize
variable importance and response curves.

# 0. QUICK Niche Theory Refresher

Today, we will be focusing on Abiotic (A) and Biotic (B) factors, but we
will not incorporate Movement (M). Movement and the accessible area is a
key component to building species distribution models and should be
addressed thoughtfully. For speed and computational reasons, we will not
be fitting our models using occurrence records from throughout the
species range, but when possible, SDMs should be fit using data from
throughout a species' entire native range.

![BAM Diagram, Soberón 2007 *Ecology
Letters*](media/BAM_Diagram.png){width="300"}

# 1. Read and prepare data

We’ll load the presence/background dataset and rename the response
variable. Then we’ll split the data into training (80%) and testing
(20%) sets, stratified by presence.
...
We’ll visualize the training and test points over the study area
(Oregon) for context.
...
We’ll create a tidymodels recipe that defines the model formula and
normalization of predictors.

In tidymodels, a 'recipe' is a structured sequence of preprocessing
steps applied to data before modeling. It allows users to define
transformations (e.g., normalization, encoding, imputation) in a
reproducible way. Recipes separate data preparation from model fitting,
ensuring consistent preprocessing across training and test sets. Here we
include normalization of predictors, which scales variables so that they
are on comparable ranges.

We're not worrying about correlation today, because our focus is
prediction, but if interpreting response curves or variable importance
were a key outcome of the model, we would need to think more carefully
about dealing with correlated variables.
...
We define a Random Forest model and specify parameters (mtry, min_n) for
tuning. The engine ranger is fast and supports permutation-based
variable importance.
...
We’ll combine the recipe and model into a workflow, then use 5-fold
...
We’ll generate predictions for the test data and compute AUC and a ROC
curve.

The AUC (Area Under the ROC Curve) measures the model's ability to
discriminate between classes. A value of 0.5 indicates random guessing,
...
specificity) across different threshold values. A model with curves
closer to the top-left corner performs better.
...
We’ll now predict probabilities across the entire study region to create
a distribution map.
...
## Extra: Challenges of unbalanced classifcations in RF models

In case you're interested, here is the output of a model that doesn't
downsample background points to match the number of presence points.
Note the model will take much longer to run because there are far more
data points (\~10,000 more background points) that the model is being
fit to. How do the results differ?

```{r extra_plotNoDownSample}
#| eval: false

# Extra info: What happens if we don't downsample?
...
rf_recipe <- recipe(presence ~ prmean + tmean + tmax + tmin + elev +
                      ...
                      forageSummer + nesting + insecticide,
                    data = train_data) %>%
 # step_downsample(presence) %>% no more downsampling
  step_normalize(all_predictors())  # Normalize predictors (optional for RF)
...
prediction_df2_nodown <- prediction_df %>%
  mutate(pred = predictions_on_map$.pred_1)

# Visualize predictions as a raster map
ggplot(prediction_df2_nodown, aes(x = x, y = y, fill = pred)) +
  geom_tile() +
  scale_fill_viridis_c() +
  coord_equal() +
  theme_minimal() +
...
The Boyce Index evaluates how well predicted suitability values match
observed presences.

It is typically used for presence-only or presence-background models,
like many SDMs. Values range from -1 to +1: - +1 indicates perfect
agreement (model predicts presences in high-suitability areas), - 0
means random prediction, and -1 indicates counter-predictive performance
(presences fall in low-suitability areas).
...
Response curves show how predicted probability changes as one variable
varies, keeping others constant.
